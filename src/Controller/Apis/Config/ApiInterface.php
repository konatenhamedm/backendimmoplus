...
$this->setStatusCode(500);
    ...
    /**
     * Set audit fields (createdBy, updatedBy) for entities using TraitEntity
     */
    protected function updateAuditFields($entity, bool $isNew = false): void
    {
        if (method_exists($entity, 'setUpdatedBy')) {
            $entity->setUpdatedBy($this->getUser());
        }
        
        if ($isNew && method_exists($entity, 'setCreatedBy')) {
            $entity->setCreatedBy($this->getUser());
        }

        if ($isNew && method_exists($entity, 'setCreatedAtValue')) {
            $entity->setCreatedAtValue();
        }
        
        if (!$isNew && method_exists($entity, 'setUpdatedAt')) {
            $entity->setUpdatedAt();
        }
    }
}
